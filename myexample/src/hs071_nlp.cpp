...
//
// $Id: HS071_NLP.cpp 2477 2014-04-08 09:57:37Z stefan $
//
...
namespace Ipopt
{
  ...
  HS071_NLP::HS071_NLP(const SmartPtr<const Journalist>& jnlst,
  ...
  {
    DBG_START_METH("HS071_NLP::HS071_NLP",
                   dbg_verbosity);
    ...
    *fint_nerror = 0;
    ...
    ASSERT_EXCEPTION(allow_discrete || (nbv == 0 && niv == 0 && nlvbi == 0 && nlvci == 0 && nlvoi == 0),
                     ...
                     "Either remove the integer variables, or change the flag in the constructor of HS071_NLP"
                    );
  ...
  void HS071_NLP::set_active_objective(Index in_obj_no)
  {
    if (hesset_called_) {
      jnlst_->Printf(J_ERROR, J_MAIN, "Internal error: HS071_NLP::set_active_objective called after HS071_NLP::call_hesset.\n");
      THROW_EXCEPTION(INVALID_TNLP, "Internal error: HS071_NLP::set_active_objective called after HS071_NLP::call_hesset.");
    }
    ASL_pfgh* asl = asl_;
    obj_no = in_obj_no;
  ...
  void HS071_NLP::call_hesset()
  {
    if (hesset_called_) {
      jnlst_->Printf(J_ERROR, J_MAIN, "Internal error: HS071_NLP::call_hesset is called twice.\n");
      THROW_EXCEPTION(INVALID_TNLP, "Internal error: HS071_NLP::call_hesset is called twice.");
    }
    ...
    if (n_obj == 0) {
      ...
      if (n_obj>1 && !set_active_objective_called_) {
        jnlst_->Printf(J_ERROR, J_MAIN,
                       "There is more than one objective function in the AMPL model, but HS071_NLP::set_active_objective has not been called.\n");
        THROW_EXCEPTION(INVALID_TNLP, "There is more than one objective function in the AMPL model, but HS071_NLP::set_active_objective has not been called");
      }
  ...
  bool HS071_NLP::get_var_con_metadata(Index n,
  ...
  bool HS071_NLP::get_constraints_linearity(Index n,
  ...
  {
    DBG_START_METH("HS071_NLP::eval_f",
                   dbg_verbosity);
  ...
  {
    DBG_START_METH("HS071_NLP::eval_grad_f",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
  ...
  {
    DBG_START_METH("HS071_NLP::eval_g", dbg_verbosity);
  ...
  {
    DBG_START_METH("HS071_NLP::eval_jac_g",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(n == n_var);
  ...
  bool HS071_NLP::eval_h(Index n, const Number* x, bool new_x,
                        ...
                        Index* jCol, Number* values)
  {
    DBG_START_METH("HS071_NLP::eval_h",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(n == n_var);
  ...
  bool HS071_NLP::internal_objval(const Number* x, Number& obj_val)
  {
    DBG_START_METH("HS071_NLP::internal_objval",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
  ...
  bool HS071_NLP::internal_conval(const Number* x, Index m, Number* g)
  {
    DBG_START_METH("HS071_NLP::internal_conval",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(m == n_con);
  ...
  bool HS071_NLP::apply_new_x(bool new_x, Index n, const Number* x)
  {
    DBG_START_METH("HS071_NLP::apply_new_x",
                   dbg_verbosity);
  ...
  void HS071_NLP::write_solution_file(const std::string& message) const
  {
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl);
  ...
  void HS071_NLP::get_discrete_info(Index& nlvb_,
                                   Index& nlvbi_,
                                   Index& nlvc_,
                                   Index& nlvci_,
                                   Index& nlvo_,
  ...
  bool HS071_NLP::get_scaling_parameters(Number& obj_scaling,
  ...
  Index HS071_NLP::get_number_of_nonlinear_variables()
  {
    ASL_pfgh* asl = asl_;
    return Max(nlvo,nlvc);
  }

  bool HS071_NLP::get_list_of_nonlinear_variables(Index num_nonlin_vars,
  ...
  char*
  HS071_NLP::get_options(const SmartPtr<OptionsList>& options,
  ...
  bool HS071_NLP::nerror_ok(void* nerror)
  {
    DBG_START_METH("HS071_NLP::nerror_ok",
                   dbg_verbosity);
